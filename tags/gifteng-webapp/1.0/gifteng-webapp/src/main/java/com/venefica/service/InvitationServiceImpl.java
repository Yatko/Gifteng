    ...
    public Long requestInvitation(InvitationDto invitationDto) throws InvitationException {
        ...
        if ( useEmailSender ) {
            try {
                ...
                String subtype = (containsZipcode ? NotificationType.SUBTYPE_INVITATION_REQUEST_RANGE : NotificationType.SUBTYPE_INVITATION_REQUEST_OTHERS);
                
                if ( !emailSender.sendNotification(NotificationType.INVITATION_REQUEST, subtype, email, vars) ) {
                    logger.error("Email send exception at invitation request");
                    throw new InvitationException(MailException.EMAIL_SEND_ERROR, "Could not send invitation mail!");
                }
            } catch ( Exception ex ) {
